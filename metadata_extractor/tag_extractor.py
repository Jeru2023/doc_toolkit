...
class TagExtractor:
    ...
    @staticmethod
    def extract(text, top_k=5, text_rank=False):
        if text_rank:
            tags = jieba.analyse.textrank(text, topK=top_k, withWeight=False, allowPOS=POS)
        else:
            # TF-IDF
...
if __name__ == '__main__':
    ...
    tags = tag_extractor.extract(text, text_rank=True)
    print(tags)
