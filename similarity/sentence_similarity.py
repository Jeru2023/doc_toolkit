...
class SentenceSimilarity:
    def __init__(self):
        self.similarity_pipeline = pipeline(Tasks.sentence_similarity, model=MODEL_PATH)

    @timer
    def compare(self, text_a, text_b):
        return self.similarity_pipeline(input=(text_a, text_b))
...
if __name__ == '__main__':
    ...
    其中，STDiT 每层的结构如下图所示。
    它用串行的方式在二维的空间注意力模块上叠加一维的时间注意力模块，用于建模时序关系。然后在时间注意力模块之后，交叉注意力模块用于对齐文本的语意。
