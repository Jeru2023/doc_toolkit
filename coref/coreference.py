...
class Coreference(BertPreTrainedModel):
    ...
    def __init__(self, config, coref_task_config):
        ...
        if self.config["use_features"]:
            self.span_width_embedding = nn.Embedding(num_embeddings=self.config["max_span_width"],
                                                     embedding_dim=self.config["feature_size"])
        # span head Embedding(ok)
        if self.config["model_heads"]:
            print("------加入span head 信息------")
            self.masked_mention_score = nn.Sequential(nn.Linear(self.bert_config.hidden_size, 1),
                                                      Squeezer(dim=1))
        ...
        # doc类别Embedding[7,20]
        self.genres_embedding = nn.Embedding(num_embeddings=len(self.genres),
                                             embedding_dim=self.config["feature_size"])
        ...
        # 前指距离embedding
        if self.config['use_prior']:
            self.antecedent_distance_embedding = nn.Embedding(num_embeddings=10,
                                                              embedding_dim=self.config["feature_size"])
            self.antecedent_distance_linear = nn.Linear(self.config["feature_size"], 1)

        if self.config["use_metadata"]:
            # [2,20]
            self.same_speaker_embedding = nn.Embedding(num_embeddings=2,
                                                       embedding_dim=self.config["feature_size"])
        if self.config["use_features"]:
            self.antecedent_offset_embedding = nn.Embedding(num_embeddings=10,
                                                            embedding_dim=self.config["feature_size"])
        if self.config['use_segment_distance']:
            self.segment_distance_embedding = nn.Embedding(num_embeddings=self.config['max_training_sentences'],
                                                           embedding_dim=self.config["feature_size"])
    ...
    def tensorize_example(self, example, is_training):
        ...
        speaker_dict = self.get_speaker_dict(flatten(speakers))
    ...
    def get_candidate_labels(candidate_starts, candidate_ends, labeled_starts, labeled_ends, labels):
        ...
        same_start = torch.eq(to_cuda(labeled_starts.view(-1, 1)), to_cuda(candidate_starts.view(1, -1)))
        # [num_labeled, num_candidates]
        same_end = torch.eq(to_cuda(labeled_ends.view(-1, 1)), to_cuda(candidate_ends.view(1, -1)))
        # [num_labeled, num_candidates]
    ...
    def batch_gather(emb, indices):
        ...
        if len(emb.shape) > 2:
            ...
            emb_size = 1
        flattened_emb = emb.view(batch_size * seq_len, emb_size)
        offset = to_cuda((torch.arange(batch_size) * seq_len).view(-1, 1))
